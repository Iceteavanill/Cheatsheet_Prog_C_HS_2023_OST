...
\begin{center}
    \begin{tabular}{|c|c|c|} \hline  
        Grösse        & Genauer Wert                & näherungswert \\ \hline  \rule{0pt}{2.5ex}
        Kilobyte (KB) & $2^{10}$ Bytes = 1024 Bytes & $10^3$ Bytes  \\ \hline  \rule{0pt}{2.5ex}
        Megabyte (MB) & $2^{20}$ Bytes = 1024 KB    & $10^6$ Bytes  \\ \hline  \rule{0pt}{2.5ex}
        Gigabyte (GB) & $2^{30}$ Bytes = 1024 MB    & $10^9$ Bytes  \\ \hline 
    \end{tabular}
\end{center}
...
Hexadezimal erlaubt 16 Ziffern (0 - 9, A - F). 
Binär stellen 4 Bits eine Hex Zahl dar.
\[
...
$n^0 \cdot x_1 + n^1 \cdot x_2 + n^2 \cdot x_3 + ....$ wobei n die Zahlenbasis ist.

\subsubsection{Umrechnung dezimal zu beliebigem Zahlensystem}

Die umzurechnende Zahl muss durch die Zahlenbasis mit Rest dividiert werden. 
Der Rest ist dann die erste Ziffer der entstehenden Zahl. 
Der erhaltene Quotient muss erneut dividiert werden und man erhält dann die folgenden Ziffern. 
Wenn der Quotient 0 erreicht hat, ist die Umrechnung fertig. 
