...
Der gesamte Code wird auf mehrere Module aufgeteilt, um die Übersichtlichkeit und Wiederverwendbarkeit zu verbessern. 
Ein Modul besteht in der Regel aus einer .h- und einer .c-Datei.

Die Schnittstelle des Moduls wird in der .h-Datei (Header) definiert und dokumentiert. 
Sie enthält: Prototypen für alle Funktionen, welche die Schnittstelle nach aussen bilden, Dokumentation wie diese Funktionen benutzt werden, structs, enums, unions, typedef, Präprozessormakros. 
Die Implementationen sind in der .c Datei

\subsubsection{Information hiding}

Information hiding bedeutet: nur so viele Informationen nach Aussen geben wie nötig. 
Daher globale Variablen sparsam verwenden. 
Diese Strategie macht Code sicherer und weniger fehleranfällig.
...
Um Mehrfachincludes zu verhindern, gibt es sogenannte \textbf{Includeguards}
\begin{lstlisting}[language = c]
#ifndef switch_h
#define switch_h
... //implementation
...
Das Schlüsselwort \textbf{inline} ermöglicht es dem Compiler, den Funktionsaufruf komplett wegzuoptimieren. 
Es gibt jedoch keine Garantie, dass er dies auch tut! Es wird häufig zusammen mit static verwendet.
...
\textit{Es wird angenommen das clang verwendet wird:}
