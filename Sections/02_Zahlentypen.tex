...
\begin{tabular}{ll}
    \textbf{char}       & immer 8 Bit (Als einziger Typ \textbf{IMMER} 1 Byte)\\
    \textbf{short}      & für kleine ganzzahlige Werte (mind. 16 Bit)\\
    \textbf{int}        & effizienteste Grösse für Prozessor (mind. 16 Bit)\\
    \textbf{long}       & für grosse ganze werte (mind. 32 Bit)\\
    \textbf{long long}  & für sehr grosse ganze werte (mind. 64 Bit)   \\
    \textbf{size\_t}    & mindestens so gross wie die maximale Anzahl \\
                        & Adressen, nie negativ
\end{tabular}
\vspace{2mm}

Ganzzahltypen können \textbf{signed} (Vorzeichenbehaftet) oder \textbf{unsigned} sein. 
Wenn nicht definiert, meist signed ausser Datentyp \textbf{char}, dieser ist nicht standardisiert.
...
\textbf{Stdint} stellt vordefinierte Typen zur Verfügung welche Plattform unabhängige feste Grössen haben. z.B. :
\begin{itemize}[itemsep=1pt, parsep=0pt]
...
Eine variable mit int Datentyp typischerweise der effizienteste.

\subsection{Gleitpunktzahl}

Gleitpunktzahlen speichern Zahlen wissenschaftlicher Schreibweise. 
Dabei treten allerdings fast immer Rundungsfehler auf.
Daher \textbf{nie auf Gleichheit testen}, sondern auf einen kleinen Bereich.
Sie brauchen generell viel brauchen viel Rechenleistung aber sie können auch Werte wie $\pm$ inf, $\pm$ 0, NaN (not a number) darstellen.
...
\subsection{Enum als Aufzählungstyp}
...
Die Valuedeklaration kann ausgelassen werden. 
Der Compiler vergibt von 0 aufsteigend automatisch Werte. 
Es kann strategisch eine Zahl dazwischen gesetzt werden, um gewisse numerische Zahlen zu Erhalten (sehr spezifisch).
...
\subsection{Enum als Ganzzahkonstanten}

Man kann per Enum auch Ganzzahlkonstanten definieren. 
Sie sind sicherer als ein \verb|#define| und erlauben auch das Definieren von grössen eines Arrays:
...
Wenn Pointer Referenziert (\textbf{Operator} : \verb|&|) werden, liest man die Adresse, auf die der Pointer zeigt.

Wenn Pointer dereferenziert (\textbf{Operator} : \verb|*|) werden, liest man den Wert, auf der gezeigt wird.
...
Ausnahme davon sind \textbf{Voidpointer}. 
...
Daher müssen unbekannte Pointer \textbf{immer} auf NULL geprüft werden vor Verwendung.
...
Das folgende Beispiel zeigt den Zustand von der Deklaration von Abschnitt\,\ref{Pointerbasics}.
\begin{center}
...
Erlaubte Zeichen sind Buchstaben, Ziffern und Underscore(\_). 
Eine Zahl darf nicht zu beginn stehen. Umlaute sind nicht erlaubt. 
Variablennamen sind \textbf{case sensitive!}

Beispiele:

\lstinputlisting{code/deklaration.c}

Es gibt Namen, die nicht erlaubt sind, da sie vom C Standard reserviert sind. 
Diese sind:
...
Es gibt die implizite und explizite Umwandlung. 
Implizit erledigt der Compiler da er selbst ein missmatch von Typen erkennt $\rightarrow$ schlecht da es eine Warnung erzeugt.
...
Der Scope beschreibt die Sichtbarkeit von Variabeln. 
Unterschieden wird:
\begin{itemize}[itemsep=1pt, parsep=0pt]
...
Array Nummerierung \textbf{beginnen immer bei 0}. 
Sie sind so lang wie bei der Deklarierung definiert. 
...
Mit Char Arrays werden Zeichenketten realisiert. 
Char Arrays müssen immer NULL('\textbackslash 0') terminiert werden, wenn mit diesen Standardfunktionen zur Char Array Verarbeitung verwendet werden. 
Die Nullterminierung wird verwendet, um zu wissen, wann die Zeichenkette fertig ist.
...
Arrays können nicht direkt miteinander verglichen werden, in Funktionen direkt übergeben werden oder direkt auf ein anderes überspielt werden (\textbf{memcpy} implementiert solch eine Funktion).
...
Typ-Attribute definieren spezielles Variablen verhalten.
\subsubsection{const}
...
Für einen konstanten Pointer muss const nach dem * stehen. 
Sonst ist nur der Wert, auf den der Pointer zeigt read only. 

\lstinputlisting{code/typatribute.c}

Um sicherzugehen, was wirklich konstant ist, kann die Definition von rechts nach links gelesen werden. 
Was links neben einem const steht, ist wirklich const.
...
\begin{itemize}
    \item \textbf{Globale Variabel oder Funktion}\\
    Sie sind nur in der Compile Unit gültig in der sie definiert sind.
    \item \textbf{lokale Variabel}\\
...
Mittels Struct kann man verschiedene Datentypen zu einem Typ zusammenführen. 
Auch Structs kann man in einen anderen Struct verwenden.
...
Die Grösse eines Struct ist nicht immer die Summe aller Typen. 
Aufgrund von Hardwarelimitationen (Alignment) können padding bytes in der Struct eingesetzt werden welche einfach leer sind. 
Daher kann man die wirkliche Grösse nur durch sizeof herausfinden.

\subsubsection{Funktionen}

Funktionsaufrufparameter und Funktionsreturns können Structs sein. 
Allerdings ist dies nicht effizient und eine Übergabe des Pointers wäre besser da in diesem Fall auch Arrays direkt übergeben werden.

\subsection{Union}

Unions haben eine identische Syntax wie Structs, allerdings teilen \textbf{alle} Elemente den \textbf{selben} Speicherbereich.
Man kann es verwenden, um z.B. einfacher an die einzelnen Bytes einer IPv4 Adresse zu kommen.
...
\textbf{Syntax} : \verb|typedef  <Typ> <Name des neuen typ>|

\textbf{stdint.h} verwendet dies. 
